...
import edu.umb.cs680.hw14.util.FSVisitor;
...
public class Directory extends FSElement {
    ...
    public LinkedList<FSElement> getChildren(){
        ...
        Collections.sort(children, Comparator.comparing((FSElement fselement)->fselement.getName()));
    ...
    public LinkedList<File> getFiles(Comparator<FSElement> comparator){
        ...
        int cnt=0;
        for(FSElement fs:this.children){
            if(!fs.isDirectory()) {
                fileslist.add((File) fs);
                // System.out.println(fileslist.get(cnt).name);
                cnt++;
